...
export default function Login() {
  ...
  // Real email resend states for unverified users
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Real REST API login to backend
      ...
      if (res.user) {
        ...
        if (!res.user.isVerified) {
          ...
          setError("Your email address is not verified yet. Please check your inbox for the verification email.");
          return;
        }
  ...
  const handleResendVerificationMail = async () => {
    ...
    try {
      ...
      setResendSuccessMsg("A new verification email has been dispatched via Resend!");
    } catch (err) {
    ...
    <div className="min-h-screen w-full flex items-center justify-center relative overflow-hidden bg-[#090d16] text-slate-100 py-12 px-4 sm:px-6 lg:px-8">
      ...
      <div className="w-full max-w-4xl grid grid-cols-1 md:grid-cols-12 glass-card rounded-3xl overflow-hidden shadow-2xl relative z-10 border border-slate-800/80 min-h-[540px] animate-entrance">
        ...
        <div className="md:col-span-5 bg-gradient-to-tr from-brand-blue via-brand-purple to-brand-green p-8 sm:p-10 text-white flex flex-col justify-between relative overflow-hidden">
          ...
          <div className="relative z-10 my-auto flex flex-col gap-4 text-left">
            ...
            <p className="text-white/90 text-sm leading-relaxed font-medium">
              Step back into your study center. Real email verification and progress tracking in one app.
            </p>
          </div>
        ...
        <div className="md:col-span-7 p-8 sm:p-12 flex flex-col justify-center bg-slate-900/90 backdrop-blur-md animate-slide-in-right">
          ...
          {error && (
            <div className="mb-6 p-4 bg-rose-950/40 border border-rose-900 text-rose-300 rounded-2xl text-xs text-left font-semibold flex flex-col gap-2 animate-fade-in">
              ...
              {isUnverified && (
                <div className="mt-2 pt-2 border-t border-rose-900 flex flex-col gap-2">
                  <button 
                    onClick={handleResendVerificationMail}
                    disabled={resendingEmail}
                    className="px-4 py-2 bg-brand-purple text-white rounded-xl text-xs font-bold w-fit flex items-center gap-2 hover:bg-brand-purple/90 transition-colors disabled:opacity-50"
                  >
                    {resendingEmail ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Send className="w-3.5 h-3.5" />}
                    Resend Verification Email
                  </button>
                  {resendSuccessMsg && (
                    <span className="text-[10px] text-emerald-400 font-bold flex items-center gap-1">
                      <CheckCircle2 className="w-3 h-3" /> {resendSuccessMsg}
                    </span>
                  )}
                </div>
              )}
